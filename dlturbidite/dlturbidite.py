...
def deep_learning_turbidite(resdir,
    ...
    model.compile(
        ...
        optimizer=SGD(
            lr=_lr, decay=_decay, momentum=_momentum, nesterov=_nesterov),
        # optimizer=Adadelta(),
...
def save_result(savedir, model, history, test_result):
    ...
    np.savetxt(
        savedir + 'loss.txt', history.history.get('loss'), delimiter=',')
    np.savetxt(
        savedir + 'val_loss.txt',
...
if __name__ == "__main__":
    ...
    for i in range(len(testcases)):
        ...
        model, history = deep_learning_turbidite(
            resdir_case,
            x_train_sub,
            y_train_sub,
            x_test,
